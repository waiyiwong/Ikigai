...
export default function App() {
  ...
  // Auto-generate the Ikigai sentence when reaching the final step
  useEffect(() => {
    if (currentStep === 5 && !finalStatement) {
      const strength = ikigaiData.goodAt[0]?.toLowerCase() || '[Strength]';
      const passion = ikigaiData.love[0]?.toLowerCase() || '[Passion]';
      const need = ikigaiData.needs[0]?.toLowerCase() || '[Community Need]';
      const income = ikigaiData.paidFor[0]?.toLowerCase() || '[Enterprise Idea]';
      
      if (ikigaiData.goodAt.length || ikigaiData.love.length || ikigaiData.needs.length || ikigaiData.paidFor.length) {
        setFinalStatement(`I will use my skill of ${strength} and passion for ${passion} to help ${need} by ${income}.`);
      }
    }
  }, [currentStep, ikigaiData, finalStatement]);
        ...
        <div className="max-w-4xl mx-auto space-y-6">
          <div className="bg-white rounded-2xl shadow-md p-8 text-center border-t-4 border-purple-500">
            ...
            <div className="relative w-72 h-72 mx-auto my-10 pointer-events-none">
              <div className="absolute top-0 left-1/2 -translate-x-1/2 w-44 h-44 bg-pink-400/40 rounded-full mix-blend-multiply flex flex-col items-center justify-start pt-6 shadow-lg border border-pink-200 backdrop-blur-sm">
                <span className="text-pink-900 font-bold text-[10px] uppercase tracking-wider">Passions</span>
                {ikigaiData.love[0] && <span className="text-pink-950 font-semibold text-sm mt-1 truncate w-28 text-center">{ikigaiData.love[0]}</span>}
              </div>
              <div className="absolute top-[25%] -left-4 w-44 h-44 bg-blue-400/40 rounded-full mix-blend-multiply flex flex-col items-start justify-center pl-6 shadow-lg border border-blue-200 backdrop-blur-sm">
                 <span className="text-blue-900 font-bold text-[10px] uppercase tracking-wider">Strengths</span>
                 {ikigaiData.goodAt[0] && <span className="text-blue-950 font-semibold text-sm mt-1 truncate w-28 text-left">{ikigaiData.goodAt[0]}</span>}
              </div>
              <div className="absolute top-[25%] -right-4 w-44 h-44 bg-green-400/40 rounded-full mix-blend-multiply flex flex-col items-end justify-center pr-6 shadow-lg border border-green-200 backdrop-blur-sm">
                 <span className="text-green-900 font-bold text-[10px] uppercase tracking-wider">Needs</span>
                 {ikigaiData.needs[0] && <span className="text-green-950 font-semibold text-sm mt-1 truncate w-28 text-right">{ikigaiData.needs[0]}</span>}
              </div>
              <div className="absolute bottom-0 left-1/2 -translate-x-1/2 w-44 h-44 bg-yellow-400/40 rounded-full mix-blend-multiply flex flex-col items-center justify-end pb-6 shadow-lg border border-yellow-200 backdrop-blur-sm">
                 {ikigaiData.paidFor[0] && <span className="text-yellow-950 font-semibold text-sm mb-1 truncate w-28 text-center">{ikigaiData.paidFor[0]}</span>}
                 <span className="text-yellow-900 font-bold text-[10px] uppercase tracking-wider">Income</span>
              </div>
